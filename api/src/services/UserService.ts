...
export class UserService {
    ...
    async createUser(username: string, password: string): Promise<any> {
        try {
            ...
            const userExist:any[] = await elasticSearchClient.searchDocuments(this.indexName, {
                query: {
            ...
            if (userExist.length > 0) {
                const existingUser = await elasticSearchClient.getDocument(this.indexName, userExist[0]._id);
    ...
    async updateUser(userId: string, payload: UserUpdate): Promise<any> {
        try {
            const userResponse = await elasticSearchClient.updateDocument(this.indexName, userId, payload);
    ...
    generateRefreshToken(payload: any): string {
        return jwt.sign({...payload, refresh: true}, String(process.env.JWT_SECRET), {
    ...
    async syncUsers(provider: any): Promise<void> {
        try {
            let users: any = await elasticSearchClient.searchDocuments(this.indexName, {
                query: {match_all: {}},
            });
    ...
    async syncUserDetails(userInfo: any, provider: any): Promise<void> {
        ...
        const shouldRenewSubscription = notificationSubscriptionId && isExpire(notificationSubscriptionExpirationDateTime);
        const shouldCreateSubscription = !notificationSubscriptionId || notificationCallBackUrl !== process.env.NOTIFICATION_HANDLER_URL;

        if (shouldRefreshToken || shouldRenewSubscription || shouldCreateSubscription) {
            if (shouldRefreshToken) {
                const {access_token, refresh_token, expires_in} = await provider.refreshAccessToken(refreshToken);
                ...
                const subscriptionRenew = await provider.renewSubscriptionForNotification(accessToken, notificationSubscriptionId);
                ...
                const subscription = await provider.subscriptionForNotification(accessToken, userId);
